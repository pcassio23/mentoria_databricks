# ============================================================================
# Gold Layer - Variação Cambial Compra
# ============================================================================
...
@dlt.table(
  name="gold.variacao_cambial_compra",
...
@dlt.expect_or_drop("valid_cotacao", "cotacaoCompra > 0")

def variacao_cambial_compra():
    """
    Transforma os dados de cotacao de Compra no modelo analitico de variacao cambial.
    ...
    return (
        dlt.read_stream("silver.cotacao_moeda_bcb")
            ...
            .agg(
                F.max(F.when(F.col("tipoBoletim") == "Abertura", F.col("cotacaoCompra"))).alias("cotacao_abertura"),
                F.max(F.when(F.col("tipoBoletim") == "Fechamento", F.col("cotacaoCompra"))).alias("cotacao_fechamento")
            )
            .withColumn(
                "variacao_percentual",
                F.round(
